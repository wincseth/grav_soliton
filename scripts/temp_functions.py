...
def matrix_const(goo, grr, ZETA, ZETA_S, ZETA_MAX):
    ...
    print(f"XAVI (matrix const.): C[3]={C[3]}, D[3]={D[3]}, E[3]={E[3]}")
    return C, D, E
...
def finite_differences(goo, grr, ZETA_S, ZETA, ZETA_MAX):
    ...
    print(f"XAVI (kg solver): u_bar[3]={U_bar[3]}, epsilon={epsilon}")
...
def gr_RK2(epsilon, U_bar, A, B, A_start, ZETA, ZETA_S, ZETA_MAX):
    ...
    print(f"XAVI (RK): A[3]={A_array[3]}, B[3]={B_array[3]}")
...
def find_AB_root(x1, x2, fx1, fx2, tol):
    ...
    if abs(x2_new - x1_new) <= tol*(abs(x2_new) +abs(x1_new))/2:
        meets_tol = True
    print(f"XAVI (AB root): x1out={x1_new}, x2out={x2_new}, meetstol={meets_tol}")
...
def integrating_inside_out(a_array, b_array, ZETA, ZETA_S, ZETA_MAX, prev_g1, prev_g2, zeta_0):
    ...
    while error2 > 10**-6:
        ...
        while tolerance == False:
            rounds += 1
            print(f"--- In metric round {rounds}, (zeta_s={ZETA_S}):")
            prev_g1 = guess_1
            prev_g2 = guess_2
            ...
            print(f"XAVI (after RK): A01={guess_1}, A02={guess_2}, fx1={fx1}, fx2={fx2}")
        ...
        print(f"--- For eps_round: {Rounds}, zeta_s={ZETA_S}")
