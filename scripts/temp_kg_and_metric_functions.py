...
def metric_converge_AB(A0_approx, epsilon, u_bar, A, B, zeta_vals, zeta_s, zeta_max, tolerance):
    ...
    A0 = np.array([-0.1, -1, 0], dtype=np.float64)
    A0_out = np.copy(A0)
    ...
    while meets_tol == False:
        ...
        print(f"    A0 before: {A0}")
        print(f"    fx before: {fx}")
        # find fx's
        ...
        if np.isnan(np.sum(A_arrays[:, 1])) or np.isnan(np.sum(A_arrays[:, 0])):
        ...
        A0_out[1] = A0[1] - fx[1]*(A0[1] - A0[0])/(fx[1] - fx[0])

        A_arrays[:, 2], B_arrays[:, 2], R_tilde2 = metric_RK2(epsilon, u_bar, A_arrays[:, 2], B_arrays[:, 2], A0_out[1], zeta_vals, zeta_s, zeta_max)

        if abs(fx[1]) < abs(fx[0]):
            A0_out[0] = A0[1]
        else:
            A0_out[0] = A0[0]
        print(f"    A0_out: {A0_out}")
        print(f"    fx out: {fx}")

        # check for convergence
        if abs(A0_out[1] - A0_out[0]) <= tolerance*(abs(A0_out[1]) + abs(A0_out[0]))/2:
            meets_tol = True
            print(f"\n*** A0 converge met in {metric_rounds} rounds: A0={A0_out} ***\n")

    A_array_out = A_arrays[:, 1]
    B_array_out = B_arrays[:, 1]
...
def iterate_kg_and_metric(A, B, zeta_vals, zeta_s, zeta_max, A_0_guess, zeta_0):
    ...
    epsilon = -1
    N_max = len(zeta_vals)
    converge_tol = 1e-6 # same tolerance between kg/gr and root finding for A_0
    a_array = A
    b_array = B
